function [X,ndx] = ilm_sort_files(X,varargin)
    ...
    % Syntax:
    %  Y = ilm_sort_files(X)
    %  Y = ilm_sort_files(X,xpr)
    %  Y = ilm_sort_files(X,xpr,<options>)
    % [Y,ndx] = ilm_sort_files(X,...)
    %
    ...
    %    'test.m'
    %    'test_new.m'
    % ilm_sort_files(B) % Shorter names before longer (dictionary sort):
    %  ans =
    %    'test.m'
    %    'test-old.m'
    %    'test_new.m'
    ...
    %    'test10.m'
    %    'test2.m'
    % ilm_sort_files(C) % Correct numeric order, shorter names before longer:
    %  ans =
    %    'test.m'
    %    'test1.m'
    %    'test2.m'
    ...
    %    'A2-old\test.m'
    %    'A2\test.m'
    % ilm_sort_files(D) % Shorter names before longer (dictionary sort):
    %  ans =
    %    'A1archive.zip'
    %    'A1\test.m'
    %    'A2\test.m'
    ...
    % Split path into {dir,ilm_search_subdir,subsubdir,...}:
