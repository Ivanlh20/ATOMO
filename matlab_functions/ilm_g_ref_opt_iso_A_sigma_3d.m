function par = ilm_g_ref_opt_iso_A_sigma_3d(par, cube, n_iter, bb_show, bb_bgs)
    if(nargin<5)
        bb_bgs = true;
    end
    ...
    for ip=1:n_data
        ...
        x = fminsearch(@(x)fn_chi2_A_sigma(x, data_xyz), x_0, options);
        x = [max(0.1*x_0(1), x(1)), max(1, min(abs(x(2)), par.d_max/2))];
        par.data(ip, 4:5) = x;
        par.data(ip, 6) = min(par.d_max, 3.0*x(2));
        
        disp(['sigma opt. p = ', num2str(ip), '/', num2str(n_data), ' - [', num2str(x_0), ', ', num2str(x), ']'])
    end
end

function [chi_2]= fn_chi2_A_sigma(x, data)
    A = x(1);
    sigma = x(2);
