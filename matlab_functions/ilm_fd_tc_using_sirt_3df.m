function[txy] = ilm_fd_tc_using_sirt_3df(system_conf, st_data, theta, txy, dtxy)
...
function[ee] = fn_get_error(system_conf, st_data, st_rec, theta, txy)
    fw_cyl_95 = ilc_tr_2d(system_conf, st_data.fw_cyl_95, 1, 1, txy, 6, 0);
    fw_cyl_50 = ilc_tr_2d(system_conf, st_data.fw_cyl_50, 1, 1, txy, 6, 0);
    
    tr = st_data.tr+txy*ilm_rot_mat_2d(theta);
    data_t = ilm_rot_tr_2d_2_data(system_conf, st_data.data, theta, st_data.p_rc, tr, fw_cyl_95, 6, 0);
    mask_proj = ilm_rot_tr_2d_2_data(system_conf, st_data.mask_proj, theta, st_data.p_rc, tr, fw_cyl_50, 6, 0);
...
%     mask_proj = double(mask_proj>0.5).*st_data.fw_sel_tr;
