function par = ilm_g_ref_opt_iso_sigma_3d(par, cube, n_iter, bb_show, bb_bgs)
    if(nargin<5)
        bb_bgs = true;
    end
    ...
    for ip=1:n_data
        ...
        x = fminsearch(@(x)fn_chi2_sigma(x, data_xyz), x_0, options);
        x = max(1, min(abs(x), par.d_max/2));
        par.data(ip, 5) = x;
        if(par.ip_c == ip)
            d_max = max(par.dc_max, 3.0*x);
        else
            d_max = max(par.d_max, 3.0*x);
        end
...
function [chi_2]= fn_chi2_sigma(x, data)
    A = data.p(4);
    sigma = x;
