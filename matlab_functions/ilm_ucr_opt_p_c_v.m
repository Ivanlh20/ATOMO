function par = ilm_ucr_opt_p_c_v(par, n_iter, bb_show)
    if(nargin<3)
        bb_show = false;
    end
    ...
    x_0 = [par.p_c(:); par.v(:)];
    x = fminsearch(@(x)fn_chi2_p_c_v(x, par), x_0, options);
    par.p_c = reshape(x(1:3), 1, 3);
    par.v = reshape(x(4:12), [3, 3]);
    
    function [chi_2]= fn_chi2_p_c_v(x, par)
        p_c = reshape(x(1:3), 1, 3);
